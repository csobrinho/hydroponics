...
static ezo_sensor_t *find_sensor(const char *name) {
    if (strncasecmp(name, ezo_ec->type, strlen(name)) == 0) {
        return ezo_ec;
    } else if (strncasecmp(name, ezo_ph->type, strlen(name)) == 0) {
        return ezo_ph;
    }
    return NULL;
}
...
esp_err_t cmd_ezo_init(void) {
    cmd_args.module = arg_str1(NULL, NULL, "<ec|ph>", "ezo module name");
