...
static uint16_t lcd_direct_read_id(const lcd_dev_t *dev);

static esp_err_t lcd_direct_set_direction(const lcd_dev_t *dev, gpio_mode_t mode);
...
esp_err_t lcd_direct_init(lcd_dev_t *dev) {
    ...
    ESP_ERROR_CHECK(lcd_direct_set_direction(dev, GPIO_MODE_OUTPUT));

    if (dev->config.rd_io_num != GPIO_NUM_NC) {
        lcd_direct_read_id(dev);
    }
    return ESP_OK;
}
...
static inline uint16_t lcd_direct_read_data8(const lcd_dev_t *dev) {
    ...
    ESP_ERROR_CHECK(lcd_direct_set_direction(dev, GPIO_MODE_INPUT));
    RD_STROBE(dev);
    uint8_t data = 0;
    for (int i = 0; i < dev->config.data_width; ++i) {
...
static inline uint16_t lcd_direct_read_data16(const lcd_dev_t *dev) {
    return (lcd_direct_read_data8(dev) << 8) | lcd_direct_read_data8(dev);
}

static uint16_t lcd_direct_read_cmd(const lcd_dev_t *dev, uint16_t cmd) {
    ARG_ERROR_CHECK(dev->config.rd_io_num != GPIO_NUM_NC, "rd_io_num is GPIO_NUM_NC");
    ARG_ERROR_CHECK(dev->handle == NULL, "Device already initialized, cannot read anymore");

    lcd_write_cmd(dev, cmd);
    uint16_t ret = lcd_direct_read_data16(dev);
    RD_IDLE(dev);
    return ret;
}

static uint16_t lcd_direct_read_id(const lcd_dev_t *dev) {
    uint16_t id = lcd_direct_read_cmd(dev, 0x0000);
    ESP_LOGI(TAG, "Detected lcd id: 0x%04x", id);
    return id;
}

static inline void lcd_direct_write_data8(const lcd_dev_t *dev, uint8_t data) {
    GPIO.out_w1tc = data_mask_clear;
    GPIO.out_w1ts = data_mask_set[data];
    WR_STROBE(dev);
}

inline void lcd_direct_write_data16(const lcd_dev_t *dev, uint16_t data) {
    lcd_direct_write_data8(dev, data >> 8);
...
inline void lcd_direct_write_datan(const lcd_dev_t *dev, const uint16_t *buf, size_t len) {
    ...
    // len represents the len of bytes, not uint16_t.
    for (int i = 0; i < len / sizeof(uint16_t); ++i) {
        lcd_direct_write_data16(dev, *buf++);
    }
}
